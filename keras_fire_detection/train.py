...
from utils.firedetectionnet import FireDectionNet
...
# lr-find = 1 means learning rate finder mode
...
fireLabels = np.ones((fireData.shape[0],)) # positive labels
nonFireLabels = np.zeros((nonFireData.shape[0],))

# stack the fire data with the nonfire data
data = np.vstack([fireData, nonFireData])
labels = np.hstack([fireLabels, nonFireLabels])
data /= 255

# onehot encoding on the labels and acount for skew in the labeled data
...
model = FireDectionNet.build(width=128, height=128, depth=3, classes=2)
...
if args["lr_find"] >0:
    ...
    print("[INFO] learning rate finder somplete")
    print("[INFO] examine plot and adjust learning rate")
    sys.exit(0)
